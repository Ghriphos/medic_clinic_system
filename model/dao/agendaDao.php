...
class AgendaDao{
    ...
    function consultarAgenda($agenda){
        $sql = "SELECT dataehora, stats, descricao, codConsulta" . " from agenda WHERE " . "codConsulta = '".$agenda->getCodConsulta()."';";
        $result = mysqli_query($this->c,$sql);
        return $result;
    }

    function consultarListaAgenda(){
        $sql = "SELECT dataehora, stats, descricao, codConsulta ". "from agenda";
        $result = mysqli_query($this->c, $sql);
        return $result;
    }
}
